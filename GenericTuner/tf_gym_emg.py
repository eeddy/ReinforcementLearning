...
class TFGymEMG(gym.Env):
    def __init__(self, obs_space, action_space):
        ...
        self.RED   = (255,0,0)
        self.cursor_size = 7
        self.max_target_size = 200

        self.cursor = None 
        self.target = None 
        self.current_target_size = randrange(40, self.max_target_size)
    ...
    def step(self, action):
        direction = action * self._action_to_direction[self._last_class]
    ...
    def _get_obs(self):
        return np.array([self._last_mavs])
    ...
    def _render_frame(self):
        ...
        try:
            ...
            self._last_mavs = float(data.split(' ')[1]) #np.array([float(i) for i in data.split(' ')[-8:]])
        except:
            pass
